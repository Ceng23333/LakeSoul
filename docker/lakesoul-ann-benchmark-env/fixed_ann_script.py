...
class LoadConfig:
    ...
    sample_ratio: float = 0.1  # Default to 10% of data to avoid memory issues
...
def load_data(config: LoadConfig, spark: SparkSession):
    ...
    train_vecs = h5f['train'][train_indices]
    ...
    # Generate random labels (since actual labels aren't in the file)
    ...
    test_rows = [(i + len(train_rows), Vectors.dense(vec.tolist()), "test", label) 
                 for i, (vec, label) in enumerate(zip(test_vecs, test_labels))]
    ...
    # Convert vectors to array format for saving to LakeSoul
    @udf(returnType=ArrayType(FloatType()))
    def vector_to_array(v):
        return v.toArray().tolist()
    
    # Convert LSH hashes to string format (LakeSoul-compatible)
    @udf(returnType=StringType())
    def hash_to_string(hash_obj):
        if hash_obj is None:
            return None
        return str(hash_obj)
    ...
    train_with_hashes = train_with_hashes.withColumn("hashes_str", hash_to_string(col("hashes")))
    
    # Remove the complex structure columns that cause problems with LakeSoul
    train_with_hashes = train_with_hashes.drop("features").drop("hashes")
...
def query_ann(config: QueryConfig, spark: SparkSession, model_path):
    ...
    try:
        params_path = f"{config.warehouse}/{config.table_name}_params"
        params_df = spark.read.format("lakesoul").load(params_path)
        params_str = params_df.first()["params"]
        model_params = json.loads(params_str)
        
        bucket_length = model_params.get("bucket_length", config.bucket_length)
        num_hash_tables = model_params.get("num_hash_tables", config.num_hash_tables)
        
        print(f"Loaded model parameters: bucket_length={bucket_length}, num_hash_tables={num_hash_tables}")
    except Exception as e:
        print(f"Could not load model parameters: {e}")
        print(f"Using default parameters:")
        print(f"  - bucket_length: {config.bucket_length}")
        print(f"  - num_hash_tables: {config.num_hash_tables}")
    ...
    for query_idx, query_row in enumerate(queries.collect()):
        ...
        if config.compute_recall:
            ...
            # Get the set of IDs from the LSH results
            lsh_ids = set([row["train_id"] for row in lsh_results_collected])
