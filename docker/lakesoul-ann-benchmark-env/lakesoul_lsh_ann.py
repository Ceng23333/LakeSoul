...
from pyspark.sql.functions import col, udf, row_number, collect_list, array, lit
...
def load_data(config: LoadConfig, spark: SparkSession):
    """
    Load data from the HDF5 file and save to LakeSoul.
    Embedding_LSH column will be automatically generated by LakeSoul.
    Uses sampling to reduce memory usage.
    """
    print(f"\n{'='*60}")
    print(f"LOAD MODE: Preparing data for LakeSoul with automatic LSH hashing")
    print(f"{'='*60}")
    ...
    # Create schema following the exact structure from the Scala reference code
    # Don't include Embedding_LSH as it will be added by LakeSoul
    schema = StructType([
        StructField("IndexId", IntegerType(), True),
        StructField("Embedding", ArrayType(FloatType()), True, metadata={"lsh_embedding_dimension": str(config.embedding_dim), "lsh_bit_width": str(config.num_hash_tables), "lsh_rng_seed": "1234567890"}),
        StructField("Embedding_LSH", ArrayType(LongType()), True)
    ])
    
    # Create rows with ids, vectors (following Scala reference)
    # Convert double arrays to float arrays to match the reference implementation
    train_rows = [Row(
        IndexId=i,
    ...
    # Add a "split" column to identify train vs test data
    df_train = spark.createDataFrame(train_rows, schema).withColumn("split", lit("train"))
    df_test = spark.createDataFrame(test_rows, schema).withColumn("split", lit("test"))
    ...
    # Save the data to LakeSoul - Embedding_LSH will be auto-generated
    model_path = f"{config.warehouse}/{config.table_name}"
    print(f"Saving data to {model_path}...")
    print(f"LakeSoul will automatically generate LSH hashes for the Embedding column")
    ...
    print(f"Successfully saved {df.count()} records to LakeSoul")
...
def query_ann(config: QueryConfig, spark: SparkSession, model_path):
    ...
    This implementation relies on LakeSoul's automatic LSH hash generation.
    """
    print(f"\n{'='*60}")
    print(f"QUERY MODE: Searching for approximate nearest neighbors using SQL approach")
    print(f"{'='*60}")
    ...
    # Check if Embedding_LSH column exists (should be auto-generated by LakeSoul)
    if "Embedding_LSH" not in lakesoul_df.columns:
        print(f"WARNING: Embedding_LSH column not found in the data. LakeSoul may not have generated LSH hashes.")
        print(f"This could happen if the Embedding column doesn't have the proper metadata.")
        print(f"Will use direct Euclidean distance instead (slower).")
    ...
    def euclidean_distance(vec1, vec2):
        ...
        Implementation based on the calculateEuclideanDistanceUDF in MergeIntoSQLSuite.
        """
    ...
    def cosine_distance(vec1, vec2):
        ...
        The greater the value, the greater the similarity.
        Implementation based on the calculateCosineDistanceUDF in MergeIntoSQLSuite.
        """
        if vec1 is None or vec2 is None:
    ...
    def jaccard_distance(vec1, vec2):
        ...
        The smaller the value, the greater the similarity.
        Implementation based on the calculateJaccardDistanceUDF in MergeIntoSQLSuite.
        """
        if vec1 is None or vec2 is None:
        ...
        # Need to convert to strings since float values aren't hashable
        set1 = set(str(x) for x in vec1)
        set2 = set(str(x) for x in vec2)
        ...
        # Calculate Jaccard coefficient and return Jaccard distance
        jaccard_coef = intersection / union
        return 1.0 - jaccard_coef
    ...
    def hamming_distance(vec1, vec2):
        ...
        The smaller the distance, the greater the similarity.
        Implementation based on the calculateHammingDistanceUDF in MergeIntoSQLSuite.
        Counts the number of different bits between the two hash vectors.
        """
        if vec1 is None or vec2 is None:
            return sys.maxsize
        ...
        # Ensure vector lengths match
        if len(vec1) != len(vec2):
            raise ValueError("Hash vector dimensions must match")
            
        # Calculate bit differences using XOR operation
        # For each pair of hash values, XOR them and count the number of 1 bits
        distance = 0
        for h1, h2 in zip(vec1, vec2):
            # Convert both to long integers
            h1_long = int(h1)
            h2_long = int(h2)
            
            # XOR the two hash values
            xor_result = h1_long ^ h2_long
            
            # Count the number of 1 bits in the XOR result
            # This is the Hamming distance for this pair of hash values
    ...
    spark.udf.register("cosine_distance", cosine_distance)
    spark.udf.register("jaccard_distance", jaccard_distance)
    ...
    pre_rank_size = config.pre_rank_size  # Use num_hash_tables as amplification factor
    ...
    if config.compute_recall:
        ...
        train_data_collected = [(row["IndexId"], row["Embedding"]) for row in train_df.select("IndexId", "Embedding").collect()]
    ...
    for query_idx, query_row in enumerate(queries.collect()):
        query_id = query_row["IndexId"]
        query_features_array = query_row["Embedding"]
        # Check if Embedding_LSH is available (should be auto-generated by LakeSoul)
        query_hashes = query_row["Embedding_LSH"]
        ...
        # Two-stage approach following MergeIntoSQLSuite pattern:
        if query_hashes is not None and "Embedding_LSH" in train_df.columns:
            # Stage 1: First filter candidates using Hamming distance on LSH hashes
            first_stage_sql = f"""
            SELECT *
            FROM (
                SELECT
                    train_data.IndexId AS train_id,
                    train_data.Embedding AS train_features,
                    query_data.IndexId AS query_id,
                    query_data.Embedding AS query_features,
                    train_data.Embedding_LSH AS train_lsh,
                    query_data.Embedding_LSH AS query_lsh,
                    hamming_distance(train_data.Embedding_LSH, query_data.Embedding_LSH) AS hamming_dist,
                    ROW_NUMBER() OVER (PARTITION BY query_data.IndexId ORDER BY hamming_distance(train_data.Embedding_LSH, query_data.Embedding_LSH) ASC) AS rank
                FROM train_data
                CROSS JOIN query_data
                WHERE query_data.IndexId = {query_id}
            ) ranked
            WHERE rank <= {pre_rank_size}
            """
            
            first_stage_results = spark.sql(first_stage_sql)
            first_stage_results.createOrReplaceTempView("first_stage_results")
            
            # Stage 2: Refine the results using Euclidean distance
            second_stage_sql = f"""
            SELECT *
            FROM (
                SELECT
                    ...
                    ROW_NUMBER() OVER (PARTITION BY query_id ORDER BY euclidean_distance(train_features, query_features) ASC) AS rank
                FROM first_stage_results
            ) reranked
            WHERE rank <= {config.topk}
            """
            
            final_results = spark.sql(second_stage_sql)
        else:
            # If no LSH hashes available, use direct Euclidean distance (slower but works)
            print("WARNING: No Embedding_LSH found in data. Using direct Euclidean distance (slower).")
            direct_sql = f"""
            SELECT *
            FROM (
                SELECT
                    ...
                    query_data.IndexId AS query_id,
                    query_data.Embedding AS query_features,
                    euclidean_distance(train_data.Embedding, query_data.Embedding) AS exact_distance,
                    ROW_NUMBER() OVER (PARTITION BY query_data.IndexId ORDER BY euclidean_distance(train_data.Embedding, query_data.Embedding) ASC) AS rank
                FROM train_data
                CROSS JOIN query_data
                WHERE query_data.IndexId = {query_id}
            ) ranked
...
def main():
    ...
    parser.add_argument("--pre-rank-size", type=int, help="Number of candidates to pre-rank")
    ...
    if args.mode == "load":
        config = LoadConfig(
            ...
            sample_ratio=args.sample_ratio or 1.0,
        ...
        config = QueryConfig(
            ...
            topk=top_k,
            warehouse=args.warehouse,
            pre_rank_size=args.pre_rank_size or top_k,
